...
class Model:
    ...
    def fullEmbed(self):
        with open('abstracts-preds.json', 'r') as file, open('abstracts-preds.json', 'r') as file, open('doc_embeds_specter.json', 'r', encoding='utf-8') as embeds:
            data = json.load(file)
            embedddings = json.load(embeds)
            int_idx2pid = {}
            ...
            for key, value in data.items():
                ...
                # if i == 500:
                #    break
            doc_embeds = np.array(embedddings)

            # doc_embeds_serializable = doc_embeds.tolist()
            # # Save doc_embeds to a JSON file
            # with open('doc_embeds_specter.json', 'w') as json_file:
...
for facet in facets:
    ...
    outputFileName = f"800k-{facet}-specter.json"
